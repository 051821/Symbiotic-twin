...
5. Starts uvicorn HTTP server on container port 8000 (host mapped to 18000 in Docker)
...
#### `Dockerfile.server`
Multi-stage build for the server. Stage 1 installs Python packages. Stage 2 copies only the installed packages and source code (`server/`, `shared/`, `config/`, `metrics/`, `agents/`, `security/`). Exposes container port 8000 (published as 18000 on host via Docker Compose).
...
Simple build — installs Streamlit and dependencies. Copies only `dashboard/`. Exposes container port 8502 (published as 18502 on host via Docker Compose).
...
Access at **http://localhost:18502** after `docker-compose up`.
...
```bash
curl -X POST http://localhost:18000/classify \
